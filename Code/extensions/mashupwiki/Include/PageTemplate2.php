<?php
$CCPageTemplate=Array(
//购物
    'Shopping'=>'<!--Category={|Category|}--><!--KeyWords={|KeyWords|}--><!--PageName={|PageName|}--><!--CreatePage={|CreatePage|}--><!--DealWhere={|DealWhere|}-->
<div id="selfpagename" style="display:none">{|PageName|}</div>
<div class="fll">{|Creater|}<div class="participant" id="particip"></div>
</div>
<div class="frr">
<div class="frtitle">
<h2>{|PageName|}</h2>
</div>
<!--{|KeyWords|}-->
<div class="gbbox">
<div id="gbbox2">{|Deal|}
</div>
</div>
<div class="gbinfo">
<div class="fll">
<ul class="tabs">
	<li class="curr">关于本团购</li>
	<li>公交查询</li>
	<li>微博互动</li>
	<li>相关评论</li>
        <li>街旁店铺</li>
        <li>淘宝交易</li>
</ul>
<ul class="tabc"><i id="aboutgb" style="display:block">{|DealDetial|}</i><i id="bus"></i><i id="weibo"></i><i id="comment"></i><i id="jiepang"></i><i id="taobao"></i></ul>
</div>
</div>
</div>
[[Category:Shopping]]',

//餐饮
    'Restaurant'=>'<!--Category={|Category|}--><!--KeyWords={|KeyWords|}--><!--PageName={|PageName|}--><!--CreatePage={|CreatePage|}--><!--DealWhere={|DealWhere|}-->
<div id="selfpagename" style="display:none">{|PageName|}</div>
<div class="fll">{|Creater|}<div class="participant" id="particip"></div>
</div>
<div class="frr">
<div class="frtitle">
<h2>{|PageName|}</h2>
</div>
<!--{|KeyWords|}-->
<div class="gbbox">
<div id="gbbox2">{|Deal|}
</div>
</div>
<div class="gbinfo">
<div class="fll">
<ul class="tabs">
	<li class="curr">关于本团购</li>
	<li>公交查询</li>
	<li>微博互动</li>
	<li>相关评论</li>
        <li>街旁店铺</li>
        <li>淘宝交易</li>
</ul>
<ul class="tabc"><i id="aboutgb" style="display:block">{|DealDetial|}</i><i id="bus"></i><i id="weibo"></i><i id="comment"></i><i id="jiepang"></i><i id="taobao"></i></ul>
</div>
</div>
</div>
[[Category:Restaurant]]',
//旅游
    'Travelling'=>'<!--Category={|Category|}--><!--KeyWords={|KeyWords|}--><!--PageName={|PageName|}--><!--CreatePage={|CreatePage|}--><!--DealWhere={|DealWhere|}-->
<div id="selfpagename" style="display:none">{|PageName|}</div>
<div class="fll">{|Creater|}<div class="participant" id="particip"></div>
</div>
<div class="frr">
<div class="frtitle">
<h2>{|PageName|}</h2>
</div>
<!--{|KeyWords|}-->
<div class="gbbox">
<div id="gbbox2">{|Deal|}
</div>
</div>
<div class="gbinfo">
<div class="fll">
<ul class="tabs">
	<li class="curr">关于本团购</li>
	<li>公交查询</li>
	<li>微博互动</li>
	<li>相关评论</li>
        <li>街旁店铺</li>
        <li>淘宝交易</li>
</ul>
<ul class="tabc"><i id="aboutgb" style="display:block">{|DealDetial|}</i><i id="bus"></i><i id="weibo"></i><i id="comment"></i><i id="jiepang"></i><i id="taobao"></i></ul>
</div>
</div>
</div>
[[Category:Travelling]]',
//娱乐
    'Entertainment'=>'<!--Category={|Category|}--><!--KeyWords={|KeyWords|}--><!--PageName={|PageName|}--><!--CreatePage={|CreatePage|}--><!--DealWhere={|DealWhere|}-->
<div id="selfpagename" style="display:none">{|PageName|}</div>
<div class="fll">{|Creater|}<div class="participant" id="particip"></div>
</div>
<div class="frr">
<div class="frtitle">
<h2>{|PageName|}</h2>
</div>
<!--{|KeyWords|}-->
<div class="gbbox">
<div id="gbbox2">{|Deal|}
</div>
</div>
<div class="gbinfo">
<div class="fll">
<ul class="tabs">
	<li class="curr">关于本团购</li>
	<li>公交查询</li>
	<li>微博互动</li>
	<li>相关评论</li>
        <li>街旁店铺</li>
        <li>淘宝交易</li>
</ul>
<ul class="tabc"><i id="aboutgb" style="display:block">{|DealDetial|}</i><i id="bus"></i><i id="weibo"></i><i id="comment"></i><i id="jiepang"></i><i id="taobao"></i></ul>
</div>
</div>
</div>
[[Category:Entertainment]]',
//其它
    'Others'=>'<!--Category={|Category|}--><!--KeyWords={|KeyWords|}--><!--PageName={|PageName|}--><!--CreatePage={|CreatePage|}--><!--DealWhere={|DealWhere|}-->
<div id="selfpagename" style="display:none">{|PageName|}</div>
<div class="fll">{|Creater|}<div class="participant" id="particip"></div>
</div>
<div class="frr">
<div class="frtitle">
<h2>{|PageName|}</h2>
</div>
<!--{|KeyWords|}-->
<div class="gbbox">
<div id="gbbox2">{|Deal|}
</div>
</div>
<div class="gbinfo">
<div class="fll">
<ul class="tabs">
	<li class="curr">关于本团购</li>
	<li>公交查询</li>
	<li>微博互动</li>
	<li>相关评论</li>
        <li>街旁店铺</li>
        <li>淘宝交易</li>
</ul>
<ul class="tabc"><i id="aboutgb" style="display:block">{|DealDetial|}</i><i id="bus"></i><i id="weibo"></i><i id="comment"></i><i id="jiepang"></i><i id="taobao"></i></ul>
</div>
</div>
</div>
[[Category:Others]]',
//健康
    'HealthCare'=>'<!--Category={|Category|}--><!--KeyWords={|KeyWords|}--><!--PageName={|PageName|}--><!--CreatePage={|CreatePage|}--><!--DealWhere={|DealWhere|}-->
<div id="selfpagename" style="display:none">{|PageName|}</div>
<div class="fll">{|Creater|}<div class="participant" id="particip"></div>
</div>
<div class="frr">
<div class="frtitle">
<h2>{|PageName|}</h2>
</div>
<!--{|KeyWords|}-->
<div class="gbbox">
<div id="gbbox2">{|Deal|}
</div>
</div>
<div class="gbinfo">
<div class="fll">
<ul class="tabs">
	<li class="curr">关于本团购</li>
	<li>公交查询</li>
	<li>微博互动</li>
	<li>相关评论</li>
        <li>街旁店铺</li>
        <li>淘宝交易</li>
</ul>
<ul class="tabc"><i id="aboutgb" style="display:block">{|DealDetial|}</i><i id="bus"></i><i id="weibo"></i><i id="comment"></i><i id="jiepang"></i><i id="taobao"></i></ul>
</div>
</div>
</div>
[[Category:HealthCare]]',
//生活
    'LifeService'=>'<!--Category={|Category|}--><!--KeyWords={|KeyWords|}--><!--PageName={|PageName|}--><!--CreatePage={|CreatePage|}--><!--DealWhere={|DealWhere|}-->
<div id="selfpagename" style="display:none">{|PageName|}</div>
<div class="fll">{|Creater|}<div class="participant" id="particip"></div>
</div>
<div class="frr">
<div class="frtitle">
<h2>{|PageName|}</h2>
</div>
<!--{|KeyWords|}-->
<div class="gbbox">
<div id="gbbox2">{|Deal|}
</div>
</div>
<div class="gbinfo">
<div class="fll">
<ul class="tabs">
	<li class="curr">关于本团购</li>
	<li>公交查询</li>
	<li>微博互动</li>
	<li>相关评论</li>
        <li>街旁店铺</li>
        <li>淘宝交易</li>
</ul>
<ul class="tabc"><i id="aboutgb" style="display:block">{|DealDetial|}</i><i id="bus"></i><i id="weibo"></i><i id="comment"></i><i id="jiepang"></i><i id="taobao"></i></ul>
</div>
</div>
</div>
[[Category:LifeService]]',
// 美容
    'Cosmetology'=>'<!--Category={|Category|}--><!--KeyWords={|KeyWords|}--><!--PageName={|PageName|}--><!--CreatePage={|CreatePage|}--><!--DealWhere={|DealWhere|}-->
<div id="selfpagename" style="display:none">{|PageName|}</div>
<div class="fll">{|Creater|}<div class="participant" id="particip"></div>
</div>
<div class="frr">
<div class="frtitle">
<h2>{|PageName|}</h2>
</div>
<!--{|KeyWords|}-->
<div class="gbbox">
<div id="gbbox2">{|Deal|}
</div>
</div>
<div class="gbinfo">
<div class="fll">
<ul class="tabs">
	<li class="curr">关于本团购</li>
	<li>公交查询</li>
	<li>微博互动</li>
	<li>相关评论</li>
        <li>街旁店铺</li>
        <li>淘宝交易</li>
</ul>
<ul class="tabc"><i id="aboutgb" style="display:block">{|DealDetial|}</i><i id="bus"></i><i id="weibo"></i><i id="comment"></i><i id="jiepang"></i><i id="taobao"></i></ul>
</div>
</div>
</div>
[[Category:Cosmetology]]');

$CCPageAskQuery=Array(
        "Weibo"=>'{{ #ask: [[Category:Ontology/Microblog]]
[[Ontology/MicroblogRelatedPage::{|PageName|}]]
| ?Ontology/Avatar=avatar
| ?Ontology/MicroblogId=name
| ?Ontology/Status=status
| ?Ontology/MicroblogPublishedTime=published time
| ?Ontology/MicroblogSource=source
| format=biweibo
}}',
        "Creater"=>'{{#ask: 
            {|CreateWhere|}
| ?Ontology/Avatar=avatar
| ?Ontology/Name=name
| ?Ontology/PersonId=id
| format=bisns
| source=html
| merge=false
| limit=1
| headers=hidden
| link=none
| stype=m
|}}',
        "Participant"=>'<div id="participheader" class="hd"><p>参与者：</p></div>{{#ask: [[Category:Ontology/Person]]
[[Ontology/Participated::{|PageName|}]]
| ?Ontology/Avatar=avatar
| ?Ontology/Name=name
| ?Ontology/PersonId=id
| format=bisns
| source=html
| merge=false
| headers=hidden
| link=none
| stype=p
|}}',
        'Deal'=>'{{#ask: {|DealWhere|}
| ?Ontology/Title=title
| ?Ontology/Picture=picture
| ?Ontology/Id=id
| ?Ontology/Url=url
| ?Ontology/Original price=orgprice
| ?Ontology/Source=source
| ?Ontology/Present price=currprice
| ?Ontology/Dscription=desc
| ?Ontology/ValidFrom=timefrom
| ?Ontology/ValidThrough=timeend
| format=bigallery
| limit=21
| sort=Ontology/ValidThrough
| link=subject
| order=ascending
| source=html
|}}',
        'DealDetial'=>'{{#ask: {|DealWhere|}
| ?Ontology/Title=title
| ?Ontology/Picture=picture
| ?Ontology/Id=id
| ?Ontology/Url=url
| ?Ontology/Original price=orgprice
| ?Ontology/Source=source
| ?Ontology/Present price=currprice
| ?Ontology/Description=desc
| ?Ontology/ValidFrom=timefrom
| ?Ontology/ValidThrough=timeend
| ?Ontology/Address=address
| ?Ontology/City=city
| ?Ontology/Latitude=latitude
| ?Ontology/Longitude=longitude
| ?Ontology/Campus=campus
| format=bidetail
| limit=1
| sort=Ontology/ValidThrough
| link=subject
| order=ascending
| source=html
|}}',
'DealMap'=>'{{#ask: {|DealWhere|}
| ?Ontology/Address=address
| ?Ontology/City=city
| ?Ontology/Latitude=latitude
| ?Ontology/Longitude=longitude
| ?Ontology/Campus=campus
| format=bimap
| limit=1
| link=subject
| order=ascending
| source=html
|}}',
'Comment'=>'{{#ask: [[Category:Ontology/Comment]]
[[Ontology/CommentRelatedDeal::{|PageName|}]]
| ?Ontology/Content=content
| ?Ontology/MicroblogId=id
| ?Ontology/CommentPublishedTime=publishedTime
| ?Ontology/Source=source
| ?Ontology/Avatar=avatar
| ?Ontology/Score=score
| ?Ontology/Name=name
| format=bicomment
|}}',
'Jiepang'=>'{{#ask: [[Category:Ontology/Shop]]
[[Ontology/ShopRelatedDeal::{|PageName|}]]
| ?Ontology/ShopName=name
| ?Ontology/Address=address
| ?Ontology/City=city
| ?Ontology/PhotoUrl=url
| format=bijiepang
|}}'
);
    
