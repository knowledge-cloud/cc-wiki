...
class MUWbIGallery extends SMWResultPrinter {
	...
	/*public function getResult( SMWQueryResult $results, array $params, $outputmode ) {
		...
		self::addJavascriptAndCSS();
                ...
                $wgOut->addScriptFile( $wgScriptPath."/extensions/mashupwiki/gb/uploadify/swfobject.js");
                $wgOut->addScriptFile( $wgScriptPath."/extensions/mashupwiki/gb/uploadify/jquery.uploadify.v2.1.4.min.js");
                $wgOut->addScriptFile( $wgScriptPath."/extensions/mashupwiki/gb/jquery-photofy.min.js");
        ...
        public function getIndexUI($results,$outputmode){
            ...
            for($i=0;$i<count($results);$i++){ 
                ...
                '<div class="gbprice"> <em>'.$info['currprice'].'</em><input type="hidden" class="hcurrname" value="'.$info[0].'" ><input type="hidden" class="hcurrurl" value="'.$info['url'].'" ><input name="" type="button" value="投票"  class="btn_qkk toupiao" />&nbsp;<input name="" type="button" value="去看看" class="btn_qkk qukankan" /></div>'.
                '</li>'; 
