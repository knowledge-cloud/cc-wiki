...
class MUWbiComment extends SMWResultPrinter {
	...
	/*public function getResult( SMWQueryResult $results, array $params, $outputmode ) {
        ...
        public function getIndexUI($results,$outputmode){
            global  $wgStylePath;
            $page=false;
            $html='<div class="commentbox"><textarea class="text750"></textarea><p>评分：<select id="CommentScore"><option value="1">1.0</option><option value="2">2.0</option><option value="3">3.0</option><option value="4">4.0</option><option value="5">5.0</option></select> <input type="image" src="'.$wgStylePath.'/ccwiki/images/btn_comment.png"></p></div>';
            ...
            if(count($results)==1 && is_array($results[0]['avatar'])){
                ...
                for($i=0;$i<$this->mSep;$i++){ 
                    if($i<$imax){
                        ...
                        <div class="ffr">
                        <p>'.$info['content'][$i].'</p>
                        <span><i class="red">分数：'.$info['score'][$i].'</i><br>'.$info['publishedTime'][$i].'</span>
                        </div>
                        </div>'; 
                    }
                }
                ...
                for($i=0;$i<$this->mSep;$i++){ 
                    if($i<$imax){
                        ...
                        <div class="ffr">
                        <p>'.$info['content'].'</p>
                        <span><i class="red">分数：'.$info['score'].'</i><br>'.$info['publishedTime'].'</span>
                        </div>
                        </div>'; 
                    }
                }
