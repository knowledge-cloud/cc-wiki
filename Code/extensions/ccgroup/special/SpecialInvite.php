...
class SpecialInvite extends SpecialPage {
		...
		//add url
		$url='http://'.$ccHost.':'.$ccPort.'/'.$ccWiki.'/includes/snsSmw.php?source='.$this->sns.'&access_token='.$this->token.'&access_token_secret='.$this->token_secret;
		$ch = curl_init($url);//打开
		...
		//login Renren
                $form .= '点击登录，邀请好友：<br /><a href="http://' .$ccHost. ':' .$ccPort. '/' .$ccWiki. '/includes/renreninviteLogin.php"><input type="image" height="60" width="140" src="http://www.guanfang.info/uploads/allimg/110107/1_110107120047_1.jpg" /></a> &nbsp &nbsp';
		//login Kaixin
		$form .= '<a href="http://' .$ccHost. ':' .$ccPort. '/' .$ccWiki. '/includes/kaixininviteLogin.php"><input type="image" height="50" width="140" src="http://smt.114chn.com/Webpub/upload/091102/129016123320781250.jpg" /></a> &nbsp &nbsp';
		//login Tencen
		$form .= '<a href="http://' .$ccHost. ':' .$ccPort. '/' .$ccWiki. '/includes/qqweiboinviteLogin.php"><input type="image" height="50" width="140" src="http://open.t.qq.com/images/resource/p9.gif" /></a> &nbsp &nbsp';
                $form .= '</fieldset>';
                return $form;
        }
	...
	private function getFriends( $token, $sns, $token_secret ){
		...
		if($sns=='renren')
			$url = 'http://' .$ccHost. ':' .$ccPort. '/' .$ccWiki. '/includes/renrenFriend.php?access_token=' . $token; 
		elseif($sns=='kaixin')
			$url = 'http://' .$ccHost. ':' .$ccPort. '/' .$ccWiki. '/includes/kaixinFriend.php?access_token=' . $token; 
		elseif($sns=='qqweibo')
			$url = 'http://' .$ccHost. ':' .$ccPort. '/' .$ccWiki. '/includes/qqweiboFriend.php?oauth_token=' .$token. '&oauth_token_secret=' . $token_secret;
