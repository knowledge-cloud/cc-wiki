...
wgFSSolrURL="http://10.214.0.147";
wgFSSolrServlet="/ccwiki/extensions/EnhancedRetrieval/includes/FacetedSearch/solrproxy.php";
/**
 * @file
...
FacetedSearch.classes.FacetedSearch = function () {
	...
	function updateSearchResults() {
		...
		mSearch = $('#query').val();
var tmp=mSearch;		
		mSearch=mSearch.replace(/\s/g,"");
		
		var tmpstring=mSearch.split("or");
   var querystring="smwh_search_field:(+";
if(tmpstring.length==1){

		 document.getElementById("searchcontent").innerHTML="搜索内容:"+tmp;
...
{
   // alert(tmpstring[i]); 
   $('#searchcontent').append(tmpstring[i]+"&nbsp;&nbsp;&nbsp;&nbsp;");
...
mAjaxSolrManager.store.addByValue('q',querystring);
document.getElementById('searchcontent').style.display="";
}
	...
	//mAjaxSolrManager.store.addByValue('q',"smwh_search_field:(+海鲜*) OR smwh_search_field:(+火锅*)");
		mAjaxSolrManager.doRequest(0);
...

{
	  	var mgr = FacetedSearch.singleton.FacetedSearchInstance.getAjaxSolrManager();
			...
			mgr.store.addByValue("fq", "smwh_categories:Ontology/Deal");
		
			mgr.doRequest(0);
			
var searchinfo=decodeURI(request("s"));
				if(searchinfo=="Search+this+wiki")
				searchinfo="(+*)";
var id=request("id");
if(id==null)
id=7;

navigate(id,searchinfo);
