...
//            itemWidth = tempItem.html(shortName + '&hellip;').width();
				...
				'<div class="xfsResultModified">' + 
					'<p>'+ lang.getMessage('lastChange') +': ' + 
						String(doc[MOD_ATT])
							...
							.substring(0, 16) + 
					'</p>' + 
				'</div><input id="doccheckbox"  onclick="savechoosen()" type="checkbox" value="'+doc.smwh_title+'"  name="checkbox">';
	...
	AjaxSolr.theme.prototype.articleProperties = function(doc) {
		...
		for (var property in doc) {
			...
			var flagshow=0;
			if(plainName=="Ontology 0/address"){
		plainName="地址";
		flagshow=1;
	   
		}
		else if(plainName=="Ontology 0/picture"){
		plainName="图片";//要处理没做
		flagshow=1;
		}
		else if(plainName=="QRCUsesQueryCall"){
