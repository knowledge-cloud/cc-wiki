...
class SkinCcWiki extends SkinTemplate {
	...
	function setupSkinUserCss( OutputPage $out ) {
		global $wgHandheldStyle,$wgStylePath;

		parent::setupSkinUserCss( $out );
		// Append to the default screen common & print styles...
		$out->addStyle( 'ccwiki/css/common.css', 'screen' );
                $out->addStyle( 'ccwiki/css/style.css', 'screen' );
                $out->addStyle( 'ccwiki/css/lhgdialog.css', 'screen' );
                $out->addStyle( $wgStylePath.'/../extensions/mashupwiki/js/map.css', 'screen' );
                $out->addStyle( $wgStylePath.'/../extensions/mashupwiki/js/comment.css', 'screen' );
		if( $wgHandheldStyle ) {
...
class CcWikiTemplate extends QuickTemplate {
	var $skin;
        var $menu=Array("/extensions/FacetedSearch/facetedsearch.html?id=0"    => "购物",
                        "/extensions/FacetedSearch/facetedsearch.html?id=1"    => "餐饮",
                        "/extensions/FacetedSearch/facetedsearch.html?id=2"    => "旅游",
                        "/extensions/FacetedSearch/facetedsearch.html?id=3"    => "娱乐",
                        "/extensions/FacetedSearch/facetedsearch.html?id=4"    => "健康",
                        "/extensions/FacetedSearch/facetedsearch.html?id=5"    => "生活",
                        "/extensions/FacetedSearch/facetedsearch.html?id=5"    => "美容",
                        "/extensions/FacetedSearch/facetedsearch.html?id=6"    => "其它",
                        "/extensions/FacetedSearch/facetedsearch.html?id=7"    => "所有");
	/**
	 ...
	 */
        function listmenu($isindex,$indexhref){
            ...
            foreach ($this->menu as $url=>$title) {
                $menuhtml.='<li><a href="'.substr($indexhref, 0,strlen($indexhref)-19).$url.'">'.$title.'</a></li>';
            }
            return $menuhtml;
        }
	function execute() {
		...
		$this->html( 'headelement' );
                if($this->data['loggedin']){
                    ...
                    $islogin=true;
                }else if(isset($_COOKIE['ccwikiUserID'])){
		    $item=array('href'=>'/ccwiki/index.php?title=Special:UserLogout&returnto=Main_Page');
                    $islogin=true;
		}else{
